...
4. Call Nano Banana API for AI generation
...
from app.models.tryon import TryOnHistoryResponse, TryOnResponse
...
async def generate_tryon(
    ...
    # Priority: Nano Banana → Gemini → Fallback composite
    generated_image = None
    ai_provider = "fallback"
    try:
        generated_image = await nano_banana_client.generate_tryon(
            model_image=preprocessed_model,
            garment_image=preprocessed_garment,
        )
        ai_provider = "nano_banana"
        logger.info("Try-on generated via Nano Banana API")
    except NanoBananaError as e:
        logger.warning(f"Nano Banana API failed: {e}. Trying Gemini...")

    if generated_image is None and gemini_image_client.is_available:
        try:
    ...
    if generated_image is None:
        logger.warning("All AI APIs failed. Using fallback composite.")
