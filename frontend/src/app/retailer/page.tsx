    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <header className="bg-gray-900 border-b border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <p className="text-sm text-gray-400 mt-1">Manage your products and fashion models</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6">
            ...
            <Link href="/retailer/products" className="text-sm text-indigo-400 hover:text-indigo-300 mt-2 inline-block">
              View all
            </Link>
          </div>

          <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6">
            <p className="text-sm text-gray-400">Fashion Models</p>
            <p className="text-3xl font-bold text-white mt-1">{totalModels}</p>
            <Link href="/retailer/models" className="text-sm text-indigo-400 hover:text-indigo-300 mt-2 inline-block">
              View all
            </Link>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6">
            ...
            <p className="text-xs text-gray-500 mt-2">Ready for try-on</p>
          </div>

          <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6">
            <p className="text-sm text-gray-400">Products with Images</p>
            <p className="text-3xl font-bold text-white mt-1">
              {products.filter((p) => p.images && p.images.length > 0).length}
            </p>
            <p className="text-xs text-gray-500 mt-2">Of latest {products.length}</p>
          </div>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/retailer/products" className="text-sm text-indigo-400 hover:text-indigo-300">
              View all
            </Link>
          </div>
          {products.length === 0 ? (
                ...
                <tbody>
                  {products.slice(0, 5).map((product) => (
                    <tr key={product._id} className="border-b border-gray-800/50 hover:bg-gray-800/50">
                      ...
                      <td className="py-3 text-gray-300">{product.images?.length || 0}</td>
        ...
        <div className="bg-gray-900 rounded-xl shadow-sm border border-gray-800 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Recent Models</h2>
            <Link href="/retailer/models" className="text-sm text-indigo-400 hover:text-indigo-300">
              View all
            </Link>
          </div>
          {models.length === 0 ? (
            <p className="text-gray-400 text-sm">No models yet. Upload your first model to get started.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {models.slice(0, 5).map((model) => (
                <div key={model._id} className="text-center">
                  ...
                  <p className="text-xs text-gray-400">{model.size} - {model.height_cm}cm</p>
                </div>
              ))}
            </div>
          )}
